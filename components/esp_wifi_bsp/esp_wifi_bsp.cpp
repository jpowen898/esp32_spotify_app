...
params::Param<std::string> m_ssid{"wifi_ssid", std::string("")};
params::Param<std::string> m_password{"wifi_password", std::string("")};
...
void espwifi_Init(void)
{
    ...
    memcpy((char*) wifi_config.sta.ssid, m_ssid.get().c_str(), m_ssid.get().length());
    memcpy((char*) wifi_config.sta.password, m_password.get().c_str(), m_password.get().length());
                          ...
                          void* event_data)
{
    ...
    else if (event_id == IP_EVENT_STA_GOT_IP)
    {
        ...
        sprintf(ip, "%d.%d.%d.%d", (uint8_t) (pxip), (uint8_t) (pxip >> 8), (uint8_t) (pxip >> 16),
                (uint8_t) (pxip >> 24));
        printf("IP: %s\n", ip);
    }
